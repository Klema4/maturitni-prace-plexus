...
/**
 * Hook pro nahrání banneru kampaně do bucketu (storage route) a uložení metadat do databáze.
 * Vrací stav drag&drop, stav nahrávání a handler pro upload.
 * @param {Object} options - Nastavení hooku.
 * @param {(url: string) => void} options.onUploaded - Callback po úspěšném nahrání (vrácená URL obrázku).
 * @return {{isDragging: boolean, isUploading: boolean, uploadError: string | null, setIsDragging: (value: boolean) => void, clearUploadError: () => void, handleUpload: (fileList: FileList | File[]) => Promise<void>}} Stav a akce pro upload banneru.
 */
export function useCampaignBannerUpload(options: { onUploaded: (url: string) => void }) {
  ...
  const { upload } = useUploadFiles({
    ...
    onUploadComplete: async (result) => {
      try {
        const uploadResult = result as { files?: unknown[] };
        ...
        for (const file of uploadedFiles) {
          ...
          const key = uploadFile.objectKey || uploadFile.objectInfo?.key;
          const fileName =
            uploadFile.name || uploadFile.metadata?.fileName || key || "banner";
